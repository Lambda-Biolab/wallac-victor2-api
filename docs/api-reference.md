...
| `POST` | `/mdb/groups` | token + authoring | Create a ProtocolGroup if it doesn't exist (idempotent) |
...
Creates a ProtocolGroup if it doesn't exist (idempotent).
...
| `group_id` | int | yes | Numeric group ID |

```json
{"group_id": 10001, "name": "eLabFTW Generated", "created": true}
```

`400` if name or group_id is missing; `403 authoring_disabled` if the feature flag is not set.
...
See [`bridge-api.md`](bridge-api.md) for the bridge (`:8423`) and designer
(`:8422`) API reference.
